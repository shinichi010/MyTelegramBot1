...
async def yt_handler(upd, ctx, url, uid):
    ...
    def _get_info():
        ...
        # 💡 تحسين: تحديث ترتيب العملاء (Clients) لتجاوز حظر يوتيوب الجديد
        # tv_embedded صار يعاني من حظر قوي، الأفضل الاعتماد على ios و android_vr
        attempts = [
            (['ios'], False),
            (['android_vr'], False),
            (['android'], False),
            (['web_creator'], False),
            (['web'], True),       # مع كوكيز للمقيد
            (['mweb'], True),
        ]
    ...
    if not info:
        return await wm.edit_text(
            ...
            "يوتيوب قام بحظر الطلب.\n"
            "💡 <b>الحلول الممكنة:</b>\n"
            "• تأكد أن الفيديو عام.\n"
            "• إذا كنت تستخدم Render، قم بتحديث ملف <code>COOKIES_DATA</code>.\n"
            "• جرب إرسال الرابط بعد قليل.",
            parse_mode="HTML"
        )
...
async def auto_download(upd, ctx, url, cid, platform="🎬", max_height=1080):
    """تحميل تلقائي بأعلى جودة متوفرة (حد أقصى تم تقليله لـ 1080p لتجنب الملفات العملاقة)"""
    msg = upd.message
    wm = await msg.reply_text(f"{platform} جاري التحميل بأعلى جودة متوفرة...")
    tmp = tempfile.mkdtemp()
    
    opts = {**_base_opts(msg.message_id)}
    opts['outtmpl'] = os.path.join(tmp, '%(id)s.%(ext)s')
    
    # 💡 تحسين: إجبار استخراج صيغة mp4 وترميز h264 لضمان عمل الفيديو داخل تليجرام
    opts['format'] = (
        f'bestvideo[height<={max_height}][ext=mp4]+bestaudio[ext=m4a]/'
        f'bestvideo[height<={max_height}]+bestaudio/'
        f'best[height<={max_height}][ext=mp4]/best'
    )
    opts['format_sort'] = [f'res:{max_height}', 'vcodec:h264', 'ext:mp4']
    opts['merge_output_format'] = 'mp4'
    ...
    try:
        ...
        active_dl.pop(msg.message_id, None)
        prog_task.cancel()
        
        if fp and os.path.exists(fp):
            # 💡 التحسين الأهم: فحص وضغط الملف إذا تجاوز 50 ميجا حتى لا يفشل الرفع لتليجرام
            fp = ensure_tg_size_limit(fp) 
            
            try: await ctx.bot.edit_message_text("📤 جاري الرفع إلى تليجرام...", chat_id=cid, message_id=wm.message_id)
            except: pass
            ...
            try: await ctx.bot.delete_message(chat_id=cid, message_id=wm.message_id)
            except: pass
        else:
            try: await ctx.bot.edit_message_text("❌ فشل التحميل. الملف غير متاح أو الرابط غير صحيح.", chat_id=cid, message_id=wm.message_id)
            except: pass
        ...
        try: await ctx.bot.edit_message_text(f"❌ فشل التحميل: حدث خطأ غير متوقع.", chat_id=cid, message_id=wm.message_id)
        except: pass
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
